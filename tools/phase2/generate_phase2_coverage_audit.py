...
ASSESSMENTS: dict[str, dict[str, Any]] = {
    ...
    "VER-ISO-0003": assessment(
        PARTIAL,
        "Packaged browser/worker/CDP/NetLog/Windows endpoint instrumentation and a deterministic forty-case endpoint, URL, UNC, device, print, escape, and malformed-value corpus exercise the production project-name and SCL typed boundaries.",
        ["apps/foundation-shell/src/runtime-wire.ts", "crates/plc-engineering-wasm/src/system_bridge.rs", "tools/phase2/source_policy.py", "tools/phase2/run_isolation_counterfactual.mjs"],
        ["apps/foundation-shell/test/runtime-wire.test.ts", "tests/phase2/isolation-counterfactual.unit.mjs", "tools/phase2/test_source_policy.py"],
        [
            "The inert fuzz corpus is not yet routed through every applicable file-metadata, navigation, trace-export, and saved-project typed boundary.",
            "Controlled live-LAN topology variation is not available in the current single-host run.",
        ],
        "LANE-ISOLATION-COUNTERFACTUAL",
    ),
    "VER-ISO-0004": assessment(
        PARTIAL,
        "Virtual load targets use canonical controller identity, file handles remain broker-private, Save As byte-verifies durable replacement, and vendor import is explicitly reserved.",
        ["apps/foundation-shell/src/file-access-broker.ts", "crates/plc-engineering-wasm/src/system_bridge.rs", "crates/plc-commissioning/src/universe.rs", "crates/plc-core/src/package.rs"],
        ["apps/foundation-shell/test/file-access-broker.test.ts", "apps/foundation-shell/test/runtime-wire.test.ts", "tests/phase2/workbench-browser.e2e.mjs"],
        [
            "Chooser/open/create/replace lacks a complete adversarial proof that only side-effect-free handle metadata is inspected before acceptance.",
            "Remote/removable/device-file/API, pipe/socket, redirect/escape, print/spool, and unapproved-helper effects lack an exhaustive negative matrix.",
            "Every export surface is not tested to reject vendor/deployable artifacts.",
        ],
        "LANE-ISOLATION-COUNTERFACTUAL",
    ),
    "VER-ISO-0005": assessment(
        PARTIAL,
        "The exit gate binds evidence to exact commit/tree/source/catalog hashes and structurally rejects skipped, flaky, unavailable, stale, inconclusive, crash, and canned records.",
        ["tools/phase2/verify_phase2.py", "tools/phase2/source_policy.py"],
        ["tools/phase2/test_verify_phase2_gate.py", "tools/phase2/test_source_policy.py"],
        ["No current machine-readable runtime isolation evidence package covers every supported platform/configuration with complete bound logs."],
        "LANE-ISOLATION-COUNTERFACTUAL",
    ),
