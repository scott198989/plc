...
The selected desktop/classroom shell and supported operating systems remain BLOCKED under `PES-DEV-0009`, `PES-DEC-0002`, and OQ-0001. This model therefore states shell-independent controls; it does not assume Electron, Tauri, a browser/PWA, WebView, installer, or portable distribution.
...
| TM-05 | The selected browser/WebView/desktop shell performs background networking, crash reporting, certificate checks, update checks, or prefetching. | Violates zero egress despite application code being clean. | Packaging remains BLOCKED until background controls, signing, updater separation, permissions, and offline verification are approved; `connect-src 'none'`; all assets local. | Process/child-process syscall, DNS, and packet monitoring during full workflows on every supported platform. | `PES-SEC-0006`-`PES-SEC-0011`, `PES-DEV-0007`, `PES-DEV-0009`, `RSK-0010` |
...
| Desktop/browser shells may have platform-specific background behavior. | OQ-0001 remains BLOCKED. No platform is accepted until process-scoped zero-attempt evidence passes. |
